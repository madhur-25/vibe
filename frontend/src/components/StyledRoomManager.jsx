...
export default function StyledRoomManager({ onRoomSelect }) {
  ...
  return (
    <div className="h-screen bg-[#09090b] flex overflow-hidden font-sans text-zinc-300">
      ...
      <div className="w-72 bg-zinc-950/50 border-r border-zinc-800 flex flex-col backdrop-blur-3xl">
        <div className="p-8">
          ...
          <div className="space-y-3 mb-10">
            <button onClick={() => setShowCreateModal(true)} className="w-full flex items-center justify-between px-5 py-3.5 bg-zinc-100 text-black rounded-2xl hover:bg-white transition-all shadow-xl font-black text-[10px] uppercase tracking-[0.2em]">
              <span>Create Room</span>
              <Plus className="w-4 h-4" />
            </button>
            <button onClick={() => setShowJoinModal(true)} className="w-full flex items-center justify-between px-5 py-3.5 bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-2xl hover:bg-zinc-800 transition-all font-black text-[10px] uppercase tracking-[0.2em]">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-1">
              {['all', 'joined', 'created', 'public'].map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-left transition-all ${filter === f ? 'bg-zinc-900 text-zinc-100 border border-zinc-800' : 'text-zinc-500 hover:text-zinc-300'}`}>
                  {f} Rooms
                </button>
              ))}
            </div>
           ...
           <div className="bg-zinc-900/40 p-4 rounded-3xl border border-zinc-800/50">
              <div className="flex items-center gap-3 mb-4 text-left">
                ...
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-zinc-100 truncate">{user?.username || "Anonymous"}</p>
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden bg-[#09090b]">
        <div className="p-10 border-b border-zinc-800/50 flex flex-col md:flex-row md:items-center justify-between z-10 gap-6 bg-[#09090b]/80 backdrop-blur-md">
          ...
          <div className="relative group w-full md:w-80">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="SEARCH FREQUENCY..." className="w-full pl-12 pr-6 py-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl text-zinc-100 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600 text-[10px] font-black tracking-[0.2em]" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 relative z-10 custom-scrollbar">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 opacity-50">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest italic">Syncing Protocals...</p>
            </div>
          ) : filteredRooms.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full opacity-20">
              <Hash className="w-20 h-20 text-zinc-800 mb-6" />
              <p className="text-2xl font-black uppercase italic tracking-tighter">No transmissions detected</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 pb-10">
              {filteredRooms.map((room) => (
                <RoomCard key={room.roomId} room={room} onSelect={onRoomSelect} onDelete={handleDeleteRoom} currentUserId={user?.id || user?._id} />
              ))}
            </div>
          )}
        </div>
      ...
      {showCreateModal && <CreateRoomModal onClose={() => setShowCreateModal(false)} onSuccess={() => { setShowCreateModal(false); fetchRooms(); }} />}
      {showJoinModal && <JoinRoomModal onClose={() => setShowJoinModal(false)} onSuccess={() => { setShowJoinModal(false); fetchRooms(); }} />}
    </div>
  );
}
...
function RoomCard({ room, onSelect, onDelete, currentUserId }) {
  ...
  return (
    <div onClick={() => room.isMember && onSelect(room)} className={`relative group bg-zinc-900/20 border border-zinc-800/50 p-8 rounded-[2.5rem] transition-all duration-500 hover:bg-zinc-900 hover:border-zinc-700 hover:translate-y-[-8px] cursor-pointer overflow-hidden ${!room.isMember ? 'opacity-40 grayscale' : ''}`}>
      {isOwner && (
        <button onClick={(e) => onDelete(e, room.roomId)} className="absolute top-6 right-6 z-20 p-2 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white">
          <Trash2 className="w-4 h-4" />
        </button>
      )}

      <div className="flex items-start justify-between mb-8 relative z-10">
        <div className="w-14 h-14 bg-zinc-950 border border-zinc-800 rounded-2xl flex items-center justify-center group-hover:bg-zinc-100 transition-all duration-500">
          {room.type === 'private' ? <Lock className="w-6 h-6 text-orange-500/50 group-hover:text-black transition-colors" /> : <Hash className="w-6 h-6 text-zinc-600 group-hover:text-black transition-colors" />}
        </div>
      ...
      <div className="space-y-2 mb-10 relative z-10 text-left">
        <h3 className="text-xl font-black text-white uppercase italic tracking-tighter truncate group-hover:text-zinc-100">{room.name}</h3>
        
        {/* ID COPY BUTTON */}
        <button onClick={handleCopyId} className="flex items-center gap-2 px-3 py-1 bg-zinc-950 border border-zinc-800 rounded-lg hover:border-zinc-600 transition-all group/id">
          <span className="text-[9px] font-mono text-zinc-500 group-hover/id:text-zinc-300 transition-colors uppercase tracking-wider">
            {copied ? "COPIED" : `ID: ${room.roomId}`}
          </span>
        </button>

        <p className="text-zinc-500 text-xs font-medium line-clamp-2 min-h-[32px] leading-relaxed pt-2">{room.description || "No frequency details provided."}</p>
      </div>
    ...
    try {
      const response = await fetch(`${API_URL}/api/rooms/join/${roomId}`, {
        ...
        headers: { 
          ...
          'Authorization': `Bearer ${token}` // ENSURES TOKEN IS SENT
        },
      ...
      <div className="bg-zinc-950 border border-zinc-800/50 rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-8 text-left">
            ...
            <div>
              <h2 className="text-2xl font-black text-zinc-100 tracking-tighter uppercase italic leading-none">Join</h2>
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.2em] mt-1">Connect ID</p>
            </div>
          </div>
          ...
          <div className="space-y-6 text-left">
            <div className="group">
              ...
              <input type="text" value={roomId} onChange={(e) => setRoomId(e.target.value)} placeholder="ROOM_ID" className="w-full px-6 py-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl text-zinc-100 font-mono text-xs focus:outline-none focus:border-zinc-600 uppercase" />
            </div>
            {requiresPassword && (
              <div className="group animate-in slide-in-from-bottom-2">
                <label className="block text-[10px] font-black text-zinc-600 mb-2 uppercase tracking-[0.2em]">Key</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••" className="w-full px-6 py-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl text-zinc-100 text-xs tracking-widest focus:outline-none focus:border-zinc-600" />
              </div>
            )}
          </div>
      ...
      <div className="bg-zinc-950 border border-zinc-800/50 rounded-[2.5rem] p-10 max-w-lg w-full shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-8 text-left">
            ...
            <div>
              <h2 className="text-2xl font-black text-zinc-100 tracking-tighter uppercase italic leading-none">Create</h2>
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.2em] mt-1">New Frequency</p>
            </div>
          </div>
          {error && <div className="mb-6 p-4 bg-red-500/5 border border-red-500/20 rounded-2xl text-red-500 text-[10px] uppercase font-black">{error}</div>}
